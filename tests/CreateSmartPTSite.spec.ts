...
import newPTSitePage from '../pages/TIMS_newSitePage';
...
test.describe("PT smart site integration", () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        PTsiteObj = new newPTSitePage(page);
    ...
    test('create smart PT site in TIMS', async ({ page }) => {
        ...
        await PTsiteObj.createSmartPTSite();
       await expect(siteInfoObj.PTTIMSsiteCode).toBeVisible({ timeout: 60000 });
        
 //////////////
        TIMSsiteCodeText = await siteInfoObj.PTTIMSsiteCode.textContent();
       console.log("###### Site Code: " + TIMSsiteCodeText)
////////////////

       await siteInfoObj.updatePTSmartSite();
        
    })

   

    test("Find PT site in commander", async ({ page }) => {
