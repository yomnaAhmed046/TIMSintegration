...
readonly windValidation: Locator;
  ...
  async updateDESmartSite() {
    ...
    await this.DETIMSsiteCode.click();
...
//function to get the tims code, i want to use it in the commander search function "to-do"
async getTIMSCode(): Promise<string> {
  const timsCode = await this.DETIMSsiteCode.innerText();
  return timsCode;
}
}